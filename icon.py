...
data = []
divs = soup.find_all('div', class_='docs-icon-item')

print(divs[0].find('div', class_='docs-icon-name').get_text())

# 获取 class 为 docs-icon-name 的标签得innerHTML
for div in divs:
    name = div.find('div', class_='docs-icon-name').get_text()
    code = div.find('div', class_='docs-icon-code').get_text()
    fontclass = div.find('div', class_='docs-icon-fontclass').get_text()
    data.append({
        'name': name,
        'code': code,
        'fontclass': fontclass
